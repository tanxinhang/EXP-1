...
- θ=(256, 1.2) H=96：V*=80.348（17.6s）；J(Phase)=81.864（Δ_J=1.89%）、J(Myo)=81.920（1.96%）、J(Direct4)=80.703（0.44%）；[D2 参考] E[B]: Phase=29.899 vs CMDP* E[B]=34.569（裸 E[B] 差 -4.670 —— 只作 primal 参考，不作 solver 证书）
...
- θ=(512, 1.2) H=96：V*=124.641（17.6s）；J(Phase)=126.519（Δ_J=1.51%）、J(Myo)=126.892（1.81%）、J(Direct4)=125.559（0.74%）；[D2 参考] E[B]: Phase=36.885 vs CMDP* E[B]=38.712（裸 E[B] 差 -1.827 —— 只作 primal 参考，不作 solver 证书）
...
- θ=(1024, 1.6) H=96：V*=234.269（17.3s）；J(Phase)=237.916（Δ_J=1.56%）、J(Myo)=241.394（3.04%）、J(Direct4)=236.987（1.16%）；[D2 参考] E[B]: Phase=40.981 vs CMDP* E[B]=44.483（裸 E[B] 差 -3.502 —— 只作 primal 参考，不作 solver 证书）
...
- θ=(8192, 1.2) H=96：V*=1327.791（17.3s）；J(Phase)=1345.105（Δ_J=1.30%）、J(Myo)=1345.562（1.34%）、J(Direct4)=1344.983（1.29%）；[D2 参考] E[B]: Phase=46.686 vs CMDP* E[B]=53.463（裸 E[B] 差 -6.778 —— 只作 primal 参考，不作 solver 证书）
