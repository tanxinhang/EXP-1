# MVS-C C3a — Migration Gate + Contract Hardening（advice/005.md，FULL）
...
- Myopic-All (G2-FG)：**θ̂=(256, 0.8)**、Ê_cal[B]=31.7975 bits、feasible 8/28  ← 与 G2 一致
- Direct8：**θ̂=(256, 0.8)**、Ê_cal[B]=36.8800 bits、feasible 10/28  ← 与 G2 一致
（229.0s）
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Myopic-All (G2-FG) | (256,0.8) | 0.0756 | 0.0896 | 0.2894 | 0.3121 | FEASIBLE | 1.6587 | 5.6575 | 32.1975 |
| Direct8 | (256,0.8) | 0.0712 | 0.0849 | 0.2913 | 0.3140 | FEASIBLE | 1.5634 | 12.5075 | 37.5225 |

- paired D=E[B^FG]−E[B^D8] = -5.3250；**Hoeffding U95=-1.1710**、L95=-9.4790（n_paired=3200，D∈[−96,96]）
- **Migration anchor 复现**：E[D] 目标 −5.3250、实测 -5.3250（偏差 0.000）→ PASS；U95 目标 −1.1710、实测 -1.1710→ PASS；双方 FEASIBLE → PASS
（24.8s）

## 3. Secondary stress @ H=48（同冻结 θ̂，诚实报告 boundary）

- paired D = -5.0263；Hoeffding U95=-2.9493、L95=-7.1032；G2 目标 −5.0263（偏差 0.000）
  - Myopic-All (G2-FG)：分类=FEASIBLE、E[B]=28.1462
  - Direct8：分类=FEASIBLE、E[B]=33.1725
（20.4s）
...
- Myopic-PJ (A={next,full})：**θ̂=(256, 0.8)**、Ê_cal[B]=34.0033 bits、feasible 10/28；10/28 网格点全停退化（E[B]=0）
...
- **Myopic-All (G2-FG)**：deterministic feasible 8/28；2-point mixture 进入 QoS 象限 174 对 → deterministic grid feasible
- **Direct8**：deterministic feasible 10/28；2-point mixture 进入 QoS 象限 140 对 → deterministic grid feasible
- **Myopic-PJ (A={next,full})**：deterministic feasible 10/28；2-point mixture 进入 QoS 象限 138 对 → deterministic grid feasible
- **StaticProg (fixed SNR ladder)**：deterministic feasible 0/28；2-point mixture 进入 QoS 象限 9 对 → deterministic grid infeasible; convex-hull (2-point mixture, POINT-ESTIMATE metric) enters QoS quadrant in 9/378 pairs
...
- 4-bit N=4 **budget-reachable** (x,h) 对（BFS，H=96，真实成本 c=16+Δr）：607121 对；检查 432644 个 (x,i,probe-feasible) 支撑（region A/B/C 计数 {'A': 0, 'B': 339776, 'C': 92868}）；dominance 检查 432644（dir_feas），矛盾 0 → **PASS**（exhaustive budget-reachable certificate）（15.7s）
...
- **C3a Migration Gate**：θ̂_FG=θ̂_D8=(256,0.8) 复现 PASS；H=96 anchor（E[D]=−5.3250、U95<0、双方 FEASIBLE）复现 **PASS** → migration 通过，可进入 C3b（005 §十七：Phase-PJ 不参加判决）。
