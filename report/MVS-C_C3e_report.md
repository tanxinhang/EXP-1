...
- **G0 判定**：action-change rate=0.0000（P(Q_phase≠Q_myopic)=0.6079 仅 Q 值层激活、pruning rate=0.7607 ⇒ 差异被剪/不改变 argmin）⇒ **Phase-PJ 与 Myopic-PJ 动作处处一致 ⇒ conditional refinement 在决策层没有产生作用（010 §一 结论复现）**。（96.4s）
...
- GPE-EA (Proposed)：**θ̂=(256, 0.8)**、Ê_cal[B]=31.7975 bits、feasible 8/28
- Myopic-All：**θ̂=(256, 0.8)**、Ê_cal[B]=31.7975 bits、feasible 8/28
（calibration 308.2s）

| | θ̂ | P_FA | U95 | P_MD | U95 | 分类 | E[N_tx] | E[B_payload] | E[B] |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| GPE-EA (Proposed) | (256,0.8) | 0.0756 | 0.0896 | 0.2894 | 0.3121 | FEASIBLE | 1.6587 | 5.6575 | 32.1975 |
| Myopic-All | (256,0.8) | 0.0756 | 0.0896 | 0.2894 | 0.3121 | FEASIBLE | 1.6587 | 5.6575 | 32.1975 |

### H=96 (primary)：paired D=E[B^GPE]−E[B^MyopicAll]

- E[D]=0.0000；**paired EB U95=0.4195**（L95=-0.4195，MP Thm 4 t=log(1/δ)）；Hoeffding sanity U95=4.1540（L95=-4.1540）
- QoS：GPE FEASIBLE（U95 0.0896/0.3121）、Myopic-All FEASIBLE（U95 0.0896/0.3121）
- **G2 判定**：BIT-UNRESOLVED（EB U95≥0；方向/量级见点估计）
- 分解：E[N_tx] GPE=1.6587 vs Myopic 1.6587；E[B_payload] 5.6575 vs 5.6575；E[B|H0] 27.8275 vs 27.8275、E[B|H1] 36.5675 vs 36.5675（017 §七 secondary）

### H=48 (stress)：paired D=E[B^GPE]−E[B^MyopicAll]

- E[D]=0.0000；**paired EB U95=0.2098**（L95=-0.2098，MP Thm 4 t=log(1/δ)）；Hoeffding sanity U95=2.0770（L95=-2.0770）
- QoS：GPE FEASIBLE（U95 0.0822/0.3712）、Myopic-All FEASIBLE（U95 0.0822/0.3712）
- **G2 判定**：BIT-UNRESOLVED（EB U95≥0；方向/量级见点估计）
- 分解：E[N_tx] GPE=1.4384 vs Myopic 1.4384；E[B_payload] 5.1312 vs 5.1312；E[B|H0] 25.5994 vs 25.5994、E[B|H1] 30.6931 vs 30.6931（017 §七 secondary）
...
- H=96 (primary)（n=3200）：D∈[−96,96]；paired EB U95=0.4195、Hoeffding U95=4.1540 —— EB 界 更紧；G2 未以该 H 通过。
- H=48 (stress)（n=3200）：D∈[−48,48]；paired EB U95=0.2098、Hoeffding U95=2.0770 —— EB 界 更紧；G2 未以该 H 通过。

## 结论（010 §十二 路线）

- **G2 (H=96) 判定**：**BIT-UNRESOLVED**（E[D]=0.0000、EB U95=0.4195、Hoeffding U95=4.1540、GPE QoS FEASIBLE、Myopic QoS FEASIBLE）
...
- **G2 双 Budget 判定**：G2 H=96 BIT-UNRESOLVED；G2 H=48 BIT-UNRESOLVED/QoS-UNRESOLVED。
