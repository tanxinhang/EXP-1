...
> 生成时间: 2026-08-23 21:16:57   模式: FULL

## 0. 状态集（P0-1）：(x,h,t) augmented + 去重 + occupancy

- Set A（random trajectories，实扣成本）= 99，Set B（on-policy occupancy @ w_ep=1000）= 23，合并去重后 = **47** 个 (x,h,t)（含 occupancy 12 个）。
...
| --- | --- | --- | --- | --- | --- | --- |
| 500 | 0.6596 | 0.5167 | 2.8872 | 7.8421 | -4.9549 | -0.2307 |
| 1000 | 0.7021 | 0.5602 | 1.5786 | 7.8421 | -6.2635 | -1.3398 |
| 2000 | 0.7021 | 0.5602 | 1.2384 | 7.8421 | -6.6037 | -1.6844 |

- **held-out 选择**：selection half 上无 W 使 LCB95[P(gap≤2)] ≥ 0.80 ⇒ w_ep 取网格最大值 2000（保守）；0.80 的统计认证缺样本（014 §2），点估计曲线见上表。
- **G0 gate（validation half @ w_ep=2000，n=24）**：UCB95(E[D_s]) = 0.6040 < −1 → FAIL；LCB95[P(gap≤2)] = 0.4671 ≥ 0.80 → FAIL（P̂=0.6667，Wilson 95% [0.4671, 0.8663]）。**G0 = FAIL**（E[D] 统计显著为负 = UNCERTAIN；P(gap≤2) 的 0.80 统计认证 = UNCERTAIN（工程验收达标，正式认证交 B0.6））。

## 2. G1 — N=4 formal certification，SNR anchor，(x,h,t)（014 §1 P0-2）

- 分层：47 个 (x,h,t) 状态中 base gap ≥ 15 的有 8 个（gap = [49.9, 42.5, 26.8, 19.6, 19.0, 19.8, 18.3, 121.1]）。eb decide 只跑 stratified 状态（SNR anchor，δ_t 用状态自己的 t）。

| W | P(override) | (n_ov/n_strat) | Wilson LCB95 | mean worlds |
| --- | --- | --- | --- | --- |
| 4000 | 1.0000 | (8/8) | 0.6756 | 4000.0000 |
| 8000 | 1.0000 | (8/8) | 0.6756 | 7958.3750 |

- **G1 gate（W=8000，SNR anchor，gap≥15）**：LCB95[P(override)] = 0.6756（8/8）≥ 0.5 → PASS；**G1 = PASS**（form 证书在值得 override 时于预算内触发）。

## 3. G2 — N=8 shallow（H=40，oracle = sparse exact Q^{π_b}），SNR anchor

- N=8 状态集：random + occupancy 去重后取前 23 个 (x,h,t) （h ∈ [20, 22, 23, 40]）。

| mode | W | P(gap≤2) | Wilson LCB95 | E[D_s] | UCB95(E[D_s]) |
| --- | --- | --- | --- | --- | --- |
| Operational | 2000 | 0.6087 | 0.4079 | -59.1984 | -36.9310 |
| Formal | 2000 | 0.5652 | 0.3681 | -56.5087 | -33.0791 |

- **G2 gate（Operational @ W=2000，n=23）**：UCB95(E[D_s]) = -36.9310 < −1 → PASS；LCB95[P(gap≤2)] = 0.4079 ≥ 0.75 → FAIL（P̂=0.6087）。**G2 = FAIL**（014 §2：n=23 下 0.75 的统计认证待 B0.6 判定）。
...
- **总评：ENGINEERING ACCEPTANCE — P0 已修；P(gap≤2) 的 0.80/0.75 统计认证未在 n=47/23 达成（UNCERTAIN），按 014 §4 直接进入 B0.6 做 stratified matched-QoS 终审**
