...
- 校准 Phase-FG(8-bit)：
    θ̂ = (8192,1.6)  (E[B]=56.273, cls=INFEASIBLE, ufa=0.0422, umd=0.2446)；理由：无 FEASIBLE：U_FA<=α 中 min U_MD
- 校准 Myopic-FG(8-bit)：
...
- 校准 Phase-FG(4-bit)：
    θ̂ = (4096,1.4)  (E[B]=46.877, cls=UNCERTAIN, ufa=0.0486, umd=0.2206)；理由：无 FEASIBLE：U_FA<=α 中 min U_MD

| method | θ̂ | QoS cls | U_FA | U_MD | E[B] | E[B|H0] | E[B|H1] | E[payload] | E[N_tx] |  （@H=96）
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Phase-FG(8-bit) | (8192,1.6) | INFEASIBLE | 0.0472 | 0.2656 | 55.998 | 44.355 | 67.641 | 17.248 | 2.422 |
...
| Phase-FG(4-bit) | (4096,1.4) | INFEASIBLE | 0.0530 | 0.2409 | 46.093 | 36.407 | 55.778 | 9.002 | 2.318 |

| method | QoS cls | U_FA | U_MD | E[B] | E[payload] | E[N_tx] |  （@H=48，同冻结 θ̂）
| --- | --- | --- | --- | --- | --- | --- | --- |
| Phase-FG(8-bit) | INFEASIBLE | 0.0516 | 0.3875 | 41.310 | 13.770 | 1.721 |
| Myopic-FG(8-bit) | INFEASIBLE | 0.0617 | 0.3531 | 32.320 | 6.120 | 1.637 |
| Direct8 | INFEASIBLE | 0.0602 | 0.3569 | 37.335 | 12.445 | 1.556 |
| Phase-FG(4-bit) | INFEASIBLE | 0.0617 | 0.3506 | 33.850 | 6.770 | 1.692 |

- Phase-FG(8-bit) − Direct8（H=96，分层 paired，n=1600）：E[D]=9.558、Hoeffding U95=15.433 | no-compare；E[B] 各 55.998 vs 46.440
- Phase-FG(8-bit) − Myopic-FG(8-bit)（H=96，分层 paired，n=1600）：E[D]=12.450、Hoeffding U95=18.325 | no-compare；E[B] 各 55.998 vs 43.548
...
| method | QoS cls | U_FA | U_MD | E[B] | E[payload] | E[N_tx] |  （@H=48，同冻结 θ̂）
| --- | --- | --- | --- | --- | --- | --- | --- |
| Phase-FG(8-bit) | FEASIBLE | 0.0773 | 0.3301 | 28.425 | 9.475 | 1.184 |
| Myopic-FG(8-bit) | FEASIBLE | 0.0954 | 0.3198 | 22.038 | 2.237 | 1.238 |
| Direct8 | FEASIBLE | 0.0842 | 0.3185 | 28.200 | 9.400 | 1.175 |
| Phase-FG(4-bit) | FEASIBLE | 0.0856 | 0.3198 | 24.750 | 4.950 | 1.238 |
...
- Phase-FG(8-bit) @ matched、ρ-homotopy：63 个网格点，FEASIBLE 数 = 0，最低 U_MD@α 边 = 0.1638、对应 P_D ≈ 0.8362
...
- θ=(256, 1.2) H=96：V*=80.348（16.9s）；J(Phase)=81.864（Δ_J=1.89%）、J(Myo)=81.920（1.96%）、J(Direct4)=80.703（0.44%）；[D2 参考] E[B]: Phase=29.899 vs CMDP* E[B]=34.569（裸 E[B] 差 -4.670 —— 只作 primal 参考，不作 solver 证书）
...
- θ=(512, 1.2) H=96：V*=124.641（16.9s）；J(Phase)=126.519（Δ_J=1.51%）、J(Myo)=126.892（1.81%）、J(Direct4)=125.559（0.74%）；[D2 参考] E[B]: Phase=36.885 vs CMDP* E[B]=38.712（裸 E[B] 差 -1.827 —— 只作 primal 参考，不作 solver 证书）
- θ=(1024, 1.6) H=48：V*=287.891（0.1s）；J(Phase)=288.171（Δ_J=0.10%）、J(Myo)=300.504（4.38%）、J(Direct4)=288.171（0.10%）；[D2 参考] E[B]: Phase=31.250 vs CMDP* E[B]=30.938（裸 E[B] 差 +0.312 —— 只作 primal 参考，不作 solver 证书）
- θ=(1024, 1.6) H=96：V*=234.269（17.0s）；J(Phase)=237.916（Δ_J=1.56%）、J(Myo)=241.394（3.04%）、J(Direct4)=236.987（1.16%）；[D2 参考] E[B]: Phase=40.981 vs CMDP* E[B]=44.483（裸 E[B] 差 -3.502 —— 只作 primal 参考，不作 solver 证书）
...
- θ=(8192, 1.2) H=96：V*=1327.791（16.9s）；J(Phase)=1345.105（Δ_J=1.30%）、J(Myo)=1345.562（1.34%）、J(Direct4)=1344.983（1.29%）；[D2 参考] E[B]: Phase=46.686 vs CMDP* E[B]=53.463（裸 E[B] 差 -6.778 —— 只作 primal 参考，不作 solver 证书）
