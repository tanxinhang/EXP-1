...
- **Phase-PJ (Proposed)**：deterministic feasible 10/28 → deterministic feasible（自身网格存在 FEASIBLE 点）
- **Myopic-PJ**：deterministic feasible 10/28 → deterministic feasible（自身网格存在 FEASIBLE 点）
- **Myopic-All**：deterministic feasible 8/28 → deterministic feasible（自身网格存在 FEASIBLE 点）
- **Direct8**：deterministic feasible 10/28 → deterministic feasible（自身网格存在 FEASIBLE 点）
...
- **L2 判定**：per-method registered convex hull：见上表（每方法独立判定）；显式 mixture 认证如上述（556.5s）
...
- **L3 Controller-search**：见上表（4/5 方法网格可行；StaticProg 按 **0/7 unique** 口径报告，010 §五）。
