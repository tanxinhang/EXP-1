...
> **matched 统计不可认证诊断（C2 审计）**：N=4 弱感知下 P_D,max^q(0.05)=0.8482、目标 P_D≥0.8382（β=0.1618）；量化全融合实际可达 ~0.845-0.848 ⇒ P_D,max−P_D 余量 ~0.005-0.008，而 Wilson 95% 半宽 n=400 → ±0.036、n=800 → ±0.025 ⇒ **ε_D=0.01 的认证余量被 CI 消耗殆尽**：matched FEASIBLE 在该系统规模下**统计不可认证**（任何 θ 只会 UNCERTAIN/INFEASIBLE —— 不是搜索失败）。机制口径（β=0.40）余量 ~0.20 ≫ CI，是 C2 的可认证比较面。
...
- θ=(256, 1.2) H=96：CMDP* E[B]=34.569（V_lag=80.3 仅作 solve 证书；memo=607890、expansions=607890、14.7s）；Phase-FG exact C=29.896（rel=-13.52%）、Myopic 29.775（-13.87%）、Direct4 31.176（-9.82%）
...
- θ=(1024, 1.6) H=96：CMDP* E[B]=44.483（V_lag=234.3 仅作 solve 证书；memo=607890、expansions=607890、14.6s）；Phase-FG exact C=40.974（rel=-7.89%）、Myopic 43.109（-3.09%）、Direct4 40.121（-9.81%）
...
- θ=(1024, 1.8) H=96：CMDP* E[B]=44.778（V_lag=248.2 仅作 solve 证书；memo=607890、expansions=607890、14.4s）；Phase-FG exact C=41.085（rel=-8.25%）、Myopic 37.130（-17.08%）、Direct4 40.254（-10.10%）
